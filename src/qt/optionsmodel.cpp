...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MNEX);
    ...
    if (!settings.contains("nAnonymizemnexcoinAmount"))
        settings.setValue("nAnonymizemnexcoinAmount", 1000);

    nAnonymizemnexcoinAmount = settings.value("nAnonymizemnexcoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizemnexcoinAmount"))
        SoftSetArg("-anonymizemnexcoinamount", settings.value("nAnonymizemnexcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in mnexcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizemnexcoinAmount:
            return QVariant(nAnonymizemnexcoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizemnexcoinAmount:
            nAnonymizemnexcoinAmount = value.toInt();
            settings.setValue("nAnonymizemnexcoinAmount", nAnonymizemnexcoinAmount);
            emit anonymizemnexcoinAmountChanged(nAnonymizemnexcoinAmount);
            break;
        case CoinControlFeatures:
