...
CzMNEXWallet::CzMNEXWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMNEXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMNEXSeed_deprecated()) {
                    LogPrintf("%s: Updated zMNEX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzMNEXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMNEXCount(nCountLastUsed);
    else if (!walletdb.ReadZMNEXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMNEXWallet::Lock()
{
    seedMaster = 0;
}

void CzMNEXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMNEXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMNEX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMNEXWallet::LoadMintPoolFromDB()
{
...
void CzMNEXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMNEXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMNEXWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzMNEXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMNEX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzMNEXWallet::SeedToZMNEX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMNEXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMNEXWallet::GenerateDeterministicZMNEX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMNEXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMNEXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
