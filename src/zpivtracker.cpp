...
CzMNEXTracker::CzMNEXTracker(std::string strWalletFile)
{
...
CzMNEXTracker::~CzMNEXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMNEXTracker::Init()
{
...
bool CzMNEXTracker::Archive(CMintMeta& meta)
{
...
bool CzMNEXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMNEXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMNEXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMNEXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMNEXTracker::GetSerialHashes()
{
...
CAmount CzMNEXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMNEXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMNEXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMNEXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMNEXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMNEXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMNEXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMNEXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMNEXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMNEXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMNEXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMNEXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMNEXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMNEXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMNEXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMNEXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMNEXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
