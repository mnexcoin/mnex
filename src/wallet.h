...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MNEX at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zMNEX send
enum ZerocoinSpendStatus {
    ZMNEX_SPEND_OKAY = 0,                            // No error
    ZMNEX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMNEX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMNEX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMNEX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMNEX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMNEX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMNEX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMNEX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMNEX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMNEX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMNEX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMNEX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMNEX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMNEX_SPENT_USED_ZMNEX = 14,                      // Coin has already been spend
    ZMNEX_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZMNEX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZMNEXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    CzMNEXWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzMNEXTracker> zpivTracker;
    ...
    void setZWallet(CzMNEXWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzMNEXTracker>(new CzMNEXTracker(strWalletFile));
    }

    CzMNEXWallet* getZWallet() { return zwalletMain; }
    ...
    /** zMNEX reset */
    boost::signals2::signal<void()> NotifyzMNEXReset;
