...
public:
    ...
    bool WriteZMNEXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMNEXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMNEXSeed_deprecated(uint256& seed);
    bool EraseZMNEXSeed();
    bool EraseZMNEXSeed_deprecated();

    bool WriteZMNEXCount(const uint32_t& nCount);
    bool ReadZMNEXCount(uint32_t& nCount);
